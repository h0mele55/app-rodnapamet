...
namespace RodnaPamet.Camera2
{
    public class CameraDroid : FrameLayout, TextureView.ISurfaceTextureListener
    {
        ...
        private readonly CameraCaptureStateSessionListener _cameraCaptureStateSessionListener;
        ...
        private CameraStateListener _videoSession;
        ...
        public CameraDroid(Context context) : base(context)
        {
            ...
            _cameraCaptureStateSessionListener = new CameraCaptureStateSessionListener
            {
                Configured = OnVideoSessionConfigured,
            };

            StartRecording = new Command(() => {
                // configure the video builder so it starts recording
                PrepareMediaRecorder();
                CameraDevice.CreateCaptureSession(new List<Surface> { _previewSurface, mediaRecorder.Surface }, _cameraCaptureStateSessionListener, _backgroundHandler);
            });
            StopRecording = new Command(() => {
                // configure the video builder so it starts recording
                mediaRecorder.Stop();
                Video?.Invoke(this, videoFileName);
            });
        }
        ...
        void PrepareMediaRecorder()
        {
            ...
            videoFileName = "video-" + DateTime.Now.ToString("yymmdd-hhmmss") + ".mp4";

            var storageDir = Android.OS.Environment.GetExternalStoragePublicDirectory(Android.OS.Environment.DirectoryMovies);
            var storageFilePath = storageDir + Java.IO.File.Separator;
            videoFileName = storageFilePath + videoFileName + ".mp4";
            ...
            AutoFitTextureView surfaceTextureView = new AutoFitTextureView(Context);
            ...
            WindowManager = _context.GetSystemService(Context.WindowService).JavaCast<IWindowManager>();
        ...
        private void OnVideoSessionConfigured(CameraCaptureSession session)
        {
            ...
            {
                _previewBuilder.Set(CaptureRequest.ControlAfMode, (int)ControlAFMode.ContinuousVideo);
            }

            _previewSession.Close();
            _previewSession = session;
            _previewSession.SetRepeatingRequest(recordRequestBuilder.Build(), null, null);
        ...
        private void SetUpCameraOutputs(int width, int height)
        {
            ...
            //todo remove all this - no pictures
            readerListener.Photo += (sender, buffer) =>
            {
//                Video?.Invoke(this, buffer);
            };
        ...
        public void StartPreview()
        {
            ...
            CameraDevice.CreateCaptureSession(surfaces,
                new CameraCaptureStateSessionListener
                {
                    Failed = session =>
                    {
                    },
