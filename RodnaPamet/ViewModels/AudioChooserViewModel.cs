...
{
    public class AudioChooserViewModel : BaseViewModel
    {
        public Command SelectValuesCommand { get; }
        public Command SelectStoryCommand { get; }
        public AudioChooserViewModel(IAnimatable cont) : base(cont)
        {
        ...
        async Task ExecuteValuesCommand()
        {
            Device.BeginInvokeOnMainThread(() => {
                IsBusy = true;
                App.Current.MainPage = new AudioPage("values");
            });
        ...
        async Task ExecuteStoryCommand()
        {
            Device.BeginInvokeOnMainThread(() => {
                IsBusy = true;
                App.Current.MainPage = new AudioPage("story");
            });
